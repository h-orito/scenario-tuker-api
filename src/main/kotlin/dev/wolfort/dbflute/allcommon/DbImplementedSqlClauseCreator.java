 ...
 */
public class DbImplementedSqlClauseCreator implements SqlClauseCreator {
    ...
    protected boolean isCurrentDBDef(DBDef currentDBDef) {
        return DbDBCurrent.getInstance().isCurrentDBDef(currentDBDef);
    }

    protected boolean isInnerJoinAutoDetect() {
        return DbDBFluteConfig.getInstance().isInnerJoinAutoDetect();
    }

    protected boolean isThatsBadTimingDetect() {
        return DbDBFluteConfig.getInstance().isThatsBadTimingDetect();
    }

    protected boolean isNullOrEmptyQueryAllowed() {
        return DbDBFluteConfig.getInstance().isNullOrEmptyQueryAllowed();
    }

    protected boolean isEmptyStringQueryAllowed() {
        return DbDBFluteConfig.getInstance().isEmptyStringQueryAllowed();
    }

    protected boolean isOverridingQueryAllowed() {
        return DbDBFluteConfig.getInstance().isOverridingQueryAllowed();
    }

    protected boolean isColumnNullObjectAllowed() {
        return DbDBFluteConfig.getInstance().isColumnNullObjectAllowed();
    }

    protected boolean isColumnNullObjectGearedToSpecify() {
        return DbDBFluteConfig.getInstance().isColumnNullObjectGearedToSpecify();
    }

    protected boolean isDatetimePrecisionTruncationOfCondition() {
        return DbDBFluteConfig.getInstance().isDatetimePrecisionTruncationOfCondition();
    }

    protected boolean isDisableSelectIndex() {
        return DbDBFluteConfig.getInstance().isDisableSelectIndex();
    }
}
