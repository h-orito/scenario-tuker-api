 ...
 */
public class DbDBFluteInitializer {

    // ===================================================================================
    //                                                                          Definition
    //                                                                          ==========
    /** The logger instance for this class. (NotNull) */
    private static final Logger _log = LoggerFactory.getLogger(DbDBFluteInitializer.class);
     ...
     */
    public DbDBFluteInitializer(javax.sql.DataSource dataSource) {
        if (dataSource == null) {
    ...
    protected void standBy() {
        if (!DbDBFluteConfig.getInstance().isLocked()) {
            DbDBFluteConfig.getInstance().lock();
        }
    ...
    protected void setupDataSourceHandler(String dataSourceFqcn) { // for Spring
        final DbDBFluteConfig config = DbDBFluteConfig.getInstance();
        ...
        if (needsSpringTransactionalDataSource(dataSourceFqcn)) {
            config.unlock();
            config.setDataSourceHandler(new DbDBFluteConfig.SpringTransactionalDataSourceHandler());
        }
    }
